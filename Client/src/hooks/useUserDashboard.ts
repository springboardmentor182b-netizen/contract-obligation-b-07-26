...
export type SystemRole =
  ...
  | 'Department Head'
  | 'Employee';
export type SystemModule = 'Contracts' | 'Compliance' | 'Renewals' | 'Reports' | 'Admin Panel';
...
export type PermissionMatrix = Record<SystemRole, Record<SystemModule, PermissionLevel>>;
...
const initialActivities: AuditActivity[] = [
...
export function useUserDashboard() {
  ...
  const roleSummaries = useMemo(
    () =>
      Object.entries(permissionMatrix).map(([role, modulePermissions]) => ({
        ...
        writeCount: Object.values(modulePermissions).filter((value) => value === 'Write').length,
        readCount: Object.values(modulePermissions).filter((value) => value === 'Read').length,
        noneCount: Object.values(modulePermissions).filter((value) => value === 'None').length,
      })),
    [permissionMatrix]
  );
  ...
  const updatePermission = useCallback((role: SystemRole, module: SystemModule, level: PermissionLevel) => {
