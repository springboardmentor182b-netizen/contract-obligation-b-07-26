...
def list_users(db: Session, page: int, page_size: int, search: Optional[str], role: Optional[str], status: Optional[str]) -> schemas.UserListResponse:
    query = db.query(User)
    if search:
        like = f"%{search}%"
    ...
    return schemas.UserListResponse(items=[schemas.UserResponse.model_validate(u) for u in users], total=total)
