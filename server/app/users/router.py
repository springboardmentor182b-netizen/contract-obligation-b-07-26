...
def read_users(
    ...
    return services.list_users(db, page=page, page_size=pageSize, search=search or None, role=role or None, status=status or None)
...
def update_user(user_id: UUID, payload: schemas.UserUpdate, db: Session = Depends(get_db), _admin=Depends(require_admin)):
    try:
...
def update_role(user_id: UUID, payload: schemas.RoleUpdateRequest, db: Session = Depends(get_db), _admin=Depends(require_admin)):
    try:
...
def update_status(user_id: UUID, payload: schemas.StatusUpdateRequest, db: Session = Depends(get_db), _admin=Depends(require_admin)):
    try:
