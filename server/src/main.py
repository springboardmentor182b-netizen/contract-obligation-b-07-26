...
@api_router.post("/api/auth/logout", status_code=status.HTTP_204_NO_CONTENT)
def logout(current_user: dict[str, Any] = Depends(get_current_user)) -> None:
    session_id = current_user.get("session_id")
    if session_id:
        revoke_session_by_token(current_user["id"], session_id)
